...
def handleLine(line:str)->str:
    ...
    tab order and newlines are handled by other functions not this one
    """
    code=""
    ...
    match t:
        case "bp":
            ...
            for i in functions:
                ...
                """ 
                #add function name
                TempCode+=i.keys()[0]#functionname
                #add args inside of ():
                TempCode+=f"({str(i[i.keys()[0]['args']])}):\n"
                #add indented code
                for s in i[i.keys()[0]['code']]:TempCode+=f"\t\t{handleLine(s)}\n"
                #now add TempCode to Code before updating the final code
                code+=TempCode
            finalCode+=code+"\n"
        case "func":
            """
...
def identifyLineType(line:str)->tuple[str,list]:
    ...
    if 'bp' in line:
        ...
        functions=[functionToDict(Actualname,tuple(constructorParams),codeLines),]#This is init function 
        ...
        for i in funcs:functions.append(functionToDict(i[0],i[1],i[2]))
